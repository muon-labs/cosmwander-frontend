...
const ContractSchema: React.FC<Props> = ({ contractAddr, init_msg, codeDetails, skeleton }) => {
  const [contractTab, setContractTab] = useState<string>("instantiate");
  const { connect, address, disconnect } = useCosmos();
  const [schema, setSchema] = useState<JSONSchema | null>(codeDetails?.contractSchema as JSONSchema);
  ...
  const options = hasSchema
    ...
    : getTabsOptionsWithoutSchema(codeDetails?.codeId || 0, contractAddr);
